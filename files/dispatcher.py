...
class TaskType(str, Enum):
    ...
    AUDIO_PROCESS      = "audio_process"     # stem sep, effects
    CROSS_MODAL        = "cross_modal"       # image→mood→music pipeline
...
class RainGodDispatcher:
    ...
    Instantiate once, inject into FastAPI lifespan.
    All adapters are lazy-loaded — only imported if their env key exists.
    """
    ...
    async def _dispatch_llm(
        self, payload: dict, prefer_local: bool = True
    ) -> DispatchResult:
        """General LLM: dolphin-llama3:8b local → Groq → OpenRouter."""
    ...
    async def _dispatch_reasoning(
        ...
        """Deep reasoning: deepseek-r1:1.5b local → Groq deepseek-r1."""
        prompt = payload.get("prompt", "")
        ...
        if self._groq:
            text = await self._groq.generate(
                prompt=prompt, model="deepseek-r1-distill-llama-70b"
            )
    ...
    async def _dispatch_tools(
        ...
        """Tool-use / structured output: qwen3:4b local → Groq."""
        prompt = payload.get("prompt", "")
        schema = payload.get("schema")  # optional JSON schema for structured output

        if prefer_local and self._local:
            text = await self._local.generate(
                prompt=prompt, model="qwen3:4b", format="json" if schema else None
            )
            return DispatchResult(source="ollama:qwen3", data={"text": text})

        if self._groq:
            text = await self._groq.generate(
                prompt=prompt, model="llama3-groq-70b-8192-tool-use-preview"
            )
    ...
    async def _dispatch_vision(
        ...
        """Vision: moondream:1.8b local → Gemini Flash."""
        image_b64 = payload.get("image_b64")
        image_url = payload.get("image_url")
        question  = payload.get("question", "Describe this image in detail.")

        if prefer_local and self._local and image_url:
            text = await self._local.vision(
                model="moondream:1.8b", image_url=image_url, prompt=question
            )
            return DispatchResult(source="ollama:moondream", data={"text": text})

        if self._gemini:
            text = await self._gemini.vision(
                image_b64=image_b64, image_url=image_url, prompt=question
            )
    ...
    async def _dispatch_embed(
        ...
        """Embeddings: always nomic-embed-text local (fast, free, no quota)."""
        text = payload.get("text", "")
        if self._local:
            embedding = await self._local.embed(
                model="nomic-embed-text", text=text
            )
    ...
    async def _dispatch_image(
        self, payload: dict, prefer_local: bool = False
    ) -> DispatchResult:
        """Image gen: Comfy Cloud primary → Replicate fallback → local ComfyUI."""
        ...
        # Fall back to local ComfyUI (requires local GPU)
    ...
    async def _dispatch_video(
        ...
        """Video/AnimateDiff: Comfy Cloud → Replicate."""
        if self._comfy_cloud:
    ...
    async def _dispatch_music(
        self, payload: dict, prefer_local: bool = False
    ) -> DispatchResult:
        """Music gen: Suno primary → HuggingFace AudioCraft fallback."""
        if self._suno:
    ...
    async def _dispatch_audio(
        ...
        """Audio processing: HuggingFace Demucs for stem separation."""
        if self._hf:
    ...
    async def _dispatch_cross_modal(
        self, payload: dict, prefer_local: bool = True
    ) -> DispatchResult:
        """Cross-modal pipeline: prompt → image → vision analysis → music.

        Steps:
          1. dolphin writes visual prompt + music mood descriptor
          2. Comfy Cloud renders image
          3. moondream analyzes rendered image for mood/palette
          4. Suno generates music matching the analysis
        """
        user_prompt = payload.get("prompt", "")
        
        # Step 1: LLM decomposes the prompt
        decompose_prompt = (
        ...
        decomp = await self._dispatch_tools(
        ...
        # Step 3: Vision analysis of generated image (if URL available)
        ...
        if img_url:
            ...
            # Enrich the suno prompt with vision feedback
