...
class OllamaAdapter:
    ...
    Models available on your stack:
      dolphin-llama3:8b   — uncensored general / agent / CrewAI
    ...
    async def chat(
        ...
        """OpenAI-style messages interface."""
        payload: dict[str, Any] = {
            "model": model,
            "messages": messages,
            "stream": False,
    ...
    async def vision(
        ...
        """Send image URL to moondream for analysis."""
        # Ollama vision requires base64 — fetch and encode
        async with httpx.AsyncClient(timeout=30) as fetch_client:
            img_resp = await fetch_client.get(image_url)
