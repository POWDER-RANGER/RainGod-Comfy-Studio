...
class SunoAdapter:
    """Wraps the Suno REST API.

    Suno API is async by nature — submit → poll until status == 'complete'.
    """
    ...
    async def generate(
        ...
        """Submit a song generation and poll until complete.

        Payload keys:
          prompt   : str   — song description / style
          tags     : str   — genres/style tags
          title    : str   — (optional) song title
          duration : int   — target seconds (30 or 60)
        """
        submit_payload = {
