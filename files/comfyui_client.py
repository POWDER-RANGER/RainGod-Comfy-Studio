...
class ComfyUIClient:
    ...
    def _request(self, method: str, url: str, **kwargs: Any) -> requests.Response:
        if self._cb.is_open():
            raise RuntimeError("ComfyUI circuit breaker is OPEN — upstream is unavailable")

        last_exc: Exception = RuntimeError("No attempts made")
        for attempt in range(self._max_retries + 1):
            try:
                resp = self._session.request(
                    method, url, timeout=self._cfg.timeout, **kwargs
                )
    ...
    def health_check(self) -> bool:
        try:
            resp = self._session.get(
                f"{self._cfg.base_url}/system_stats", timeout=10
            )
    ...
    def get_image_bytes(
        ...
        resp = self._request(
            "GET", f"{self._cfg.base_url}/view", params=params
        )
        return resp.content
