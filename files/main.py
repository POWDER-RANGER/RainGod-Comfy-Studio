...
Or via start_all.sh (which handles ComfyUI + backend together).
"""
...
# 1. Dispatch router (new) — /dispatch and /dispatch/status
from .dispatch_routes import router as dispatch_router
app.include_router(dispatch_router)

# 2. Original RAINGOD backend router — /generate, /batch-generate, /health, etc.
#    Import conditionally so this file works even if ComfyUI isn't installed
try:
    from .rain_backend import app as _rain_app
    # Graft all routes from rain_backend onto this app
...
if _SWITCHBOARD.exists():
    # Serve /ui/... for any static assets next to index.html
