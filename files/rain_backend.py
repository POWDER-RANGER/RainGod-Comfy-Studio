...
class GenerateRequest(BaseModel):
    ...
    preset:          str         = Field(default="quality")
    resolution:      str         = Field(default="cover_art")
    lora_style:      str | None  = Field(default=None)
...
async def generate(
    ...
    estimated_seconds = sampler.steps * 0.3
    job_id            = str(uuid.uuid4())

    logger.info(
        "Generation queued job_id=%s prompt_id=%s preset=%s resolution=%s",
        job_id, prompt_id, req.preset, req.resolution,
    )
...
async def batch_generate(
    ...
    for req in batch_req.requests:
        try:
            single = await generate(req, background_tasks)
            results.append({"status": "queued", "prompt_id": single.prompt_id})
